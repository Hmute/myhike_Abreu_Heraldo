...
function readQuote(day) {
    db.collection("quotes").doc(day)                                                      //name of the collection and documents should matach excatly with what you have in Firestore
        .onSnapshot(dayDoc => {                                                               //arrow notation
            ...
            quoteOfDay = dayDoc.data().quote;                                                //get the quote from the data object
    ...
    db.collection(collection).orderBy("length").get()   //the collection called "hikes"
        .then(allHikes => {
            ...
            allHikes.forEach(doc => { //iterate thru each doc
                ...
                newcard.querySelector('i').id = "save-" + docID; // assign unique ID to each element
                newcard.querySelector('i').onclick = () => updateBookmark(docID); // assign onclick function to each element
...
function updateBookmark(hikeDocID) {
    currentUser.get().then(userDoc => {
        ...
        let isBookedmarked = bookmarks.includes(hikeDocID);
        console.log("isBookmarked: " + isBookedmarked);

        if (isBookedmarked) {
            currentUser.update({
                bookmarks: firebase.firestore.FieldValue.arrayRemove(hikeDocID)
            }).then(() => {
                console.log("Document successfully updated!" + hikeDocID);
